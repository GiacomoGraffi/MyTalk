...
:- ensure_loaded('./wordnet/wn_ant.pl').
:- ensure_loaded('./wordnet/wn_s.pl').
:- ensure_loaded('./wordnet/wn_hyp.pl').
...
isa(X, Y) :- 
	s_demo(Syn1, _, X, _, _, _),
	hyp_demo(Syn1, Hyper),
	s_demo(Hyper, _, H, _, _, _),
	isa(H, Y).

isa(X, Y) :- 
	s_demo(Syn1, _, X, n,_,_),
	s_demo(Syn2, _, Y, n,_,_),
	hyp_demo(Syn1, Syn2).
...
%%%%%%% replace_whitespace(Noun, Result) %%%%%%%%%%%%%%
%
% Replace all the whitespaces in a nuon
% e.g. : living thing -> living_thing
%

replace_whitespace(Noun, Result) :-
	atomic_list_concat(Words, ' ', Noun),
