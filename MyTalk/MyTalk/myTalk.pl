...
%:- ensure_loaded('./ontology/wordnet/wn.pl').
...
%%%%%%% do_hyp_assertion(Assertion) %%%%%%%%%%%%%%
%
% Given an Assertion, it assert all the hypernym of Head
% e.g. : man(bertrand) - hominid(bertrand) - living_thing(bertrand) - physical_entity(bertrand) - entity(bertrand)
%

do_hyp_assertion(Assertion) :-
	Assertion =.. [ Head, Name ],
	pn(Name, _, _, _),
	find_hyp_chain(Head, Hyps),
	assertall(Hyps, Name).
	
do_hyp_assertion(Assertion) :-					% Not interested in other cases
	Assertion =.. [ Head, Name ],
	\+ pn(Name, _, _, _).
...
Nonterminal names:
			...
			n 			Noun
			pn 			Proper Noun
			cn			Compost Noun (e.g.: 'living thing')
			pl			Place
...
vp(finite, S, GapInfo, sg) -->
	[goes],
	pn((PN^Y)^Z, _),
	[to],
	[PL],
	{create_goto_term(PL, PN, S)}.
...
cn(NP^N1) --> [W1], [W2], {cn(W1^W2, N1, NP)}.

pl(X^Y^S) --> [P], {pl(P, X^Y, S)}.
...
cn(W1^W2, X^Term, (X^true)^exists(X,Term&true)) :-
	atom_concat(W1, ' ', W),
	atom_concat(W, W2, Word),
	has_whitespace(Word),
	create_cn(Word, X^Term).
	
create_n(W, X^ --Term):- 
	Term =.. [W, X].

create_cn(W, X^ --Term):- 
	replace_whitespace(W, NEWW),
	Term =.. [NEWW, X].
	
create_goto_term(PL, PN, --Term) :-
	Term =.. [goto, PN, PL].

pl(Place, X^S, --Term2 => --Term1) :-
	Term1 =.. [S, Var, Place],
	Term2 =.. [X, Var].
